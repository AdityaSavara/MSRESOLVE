...
###OLD marginalChangeRestrictor function. The new one is below.
...
#greater than 100%
def oldMarginalChangeRestrictor(data,abscissa,dataRangeSpecifierYorN = [],datafromcsv = [],MaxAllowedDeltaYRatio=2.0, IgnorableDeltaYThreshold = 0.0001):
#### The comments in this function are no longer completely accurate since we have added some absolute values and deltas where they were not before.
#### ( Look at the version XXXXXX of the program to see what this function looked like previously )
#The current marginalChangeRestrictor function, while functional, suffers from two serious performance issues. 
#
...
def marginalChangeRestrictor(data,abscissa,MaxAllowedDeltaYRatio=2.0, IgnorableDeltaYThreshold=0.0001, cleanupSuperfluousInterpolatedRows=True):
    ...
    # This set of statments (step1-3) runs the data through all of the necessary preprocessing
