...
for indexToChangeChosenReferenceAt, massFragCombination in enumerate(massFragCombinations):
    chosenReference[1,indexToChangeChosenReferenceAt]=60#This is to change the array so the significance factor sums will be different during each iteration. If a different array is not provided each time, the significance factor sums will all be the same.
