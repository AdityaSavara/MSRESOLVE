...
UserChoices['measuredReferenceYorN']['referenceFileStandardTuningAndForm'] = [] #optional: Provides tuningCorrectionIntensity feature. Must include csv file and form. Example: ['NISTRef.csv', 'xyyy'] .     Will automatically be used for ReferenceFileExistingTuning if that variable is blank and if createMixedTuningPattern=True.
...
UserChoices['measuredReferenceYorN']['referenceFileExistingTuningAndForm'] =[] #This is the pattern that will be pattern tuning corrected. Normally should be empty list, []. Otherwise, must include csv file and form. Example: ['NISTRef.csv', 'xyyy'] .
UserChoices['measuredReferenceYorN']['referenceFileDesiredTuningAndForm'] =[] #This is what the pattern will look more like after everything is done. Normally should be empty, list, []. Otherwise, must include csv file and form. Example: ['OriginalRef.csv', 'xyyy'] .
...
UserChoices['dataAnalysisMethods']['answer'] = 'inverse'	#'inverse' or 'sls'; sls is suggested. 'autosolver' is in development.
