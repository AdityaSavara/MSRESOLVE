...
def ReferenceThresholdFilter(referenceDataArrayWithAbscissa,referenceValueThreshold):
    print("line 519", numpy.shape(referenceDataArrayWithAbscissa), numpy.shape(referenceValueThreshold))
...
def createReferencePatternWithTuningCorrection(ReferenceData, verbose=True, skip=True):
    ...
    print("line 1195", ReferenceData.provided_reference_patterns)
    if skip == False:
        print("line 1196", ReferenceData.standardized_reference_patterns)
        numpy.savetxt("Line1195.csv", ReferenceData.provided_reference_patterns, delimiter=",")
        numpy.savetxt("Line1196.csv", ReferenceData.standardized_reference_patterns, delimiter=",")
    ReferenceData.standardized_reference_patterns=StandardizeReferencePattern(ReferenceData.provided_reference_patterns)
    print("line 1197", ReferenceData.standardized_reference_patterns)
    numpy.savetxt("Line1197.csv", ReferenceData.standardized_reference_patterns, delimiter=",")
...
'''
def ReferenceInputPreProcessing(ReferenceData, verbose=True, tuningCorrection=False):
    #print("line 1252", ReferenceData.standardized_reference_patterns)
    if tuningCorrection == True:
        print("line 1254", True)
        ReferenceData = createReferencePatternWithTuningCorrection(ReferenceData, verbose=verbose, skip=True)    
    if tuningCorrection == False:
        print("line 1254", False)
        numpy.savetxt("Line1254.csv", ReferenceData.standardized_reference_patterns, delimiter=",")
        print("line 1254", ReferenceData.standardized_reference_patterns)
        #ReferenceData = createReferencePatternWithTuningCorrection(ReferenceData, verbose=verbose, skip=False)    
        numpy.savetxt("Line1255.csv", ReferenceData.standardized_reference_patterns, delimiter=",")
    print("line 1255", ReferenceData.standardized_reference_patterns)
...
def PrepareReferenceObjectsAndCorrectionValues(ReferenceData, ExperimentData, extractReferencePatternFromDataOption='no', rpcMoleculesToChange=[], rpcMoleculesToChangeMF=[[]], rpcTimeRanges=[[]], verbose=True, tuningCorrection=False):
    ...
    ReferenceData = ReferenceInputPreProcessing(ReferenceData, verbose, tuningCorrection=tuningCorrection) #Note: if implicitSLScorrection is being used, G.currentReferenceDataUnfiltered gets created inside ReferenceInputPreProcessing
...
def main():
    ...
    for referenceObjectIndex in range(len(AllMoleculesReferenceDataList)):
        ...
        PrepareReferenceObjectsAndCorrectionValues(AllMoleculesReferenceDataList[referenceObjectIndex],AllMassFragmentsExperimentData, tuningCorrection=True)
    ...
    # Reference Pattern Changer
    if G.extractReferencePatternFromDataOption == 'yes':
        for i in range(len(ReferenceDataList)):#perform for each reference pattern.
    ...
    #Tuning Corrector
    print("line 5318, normal tuning corrector loop")
    for i in range(len(ReferenceDataList)):#perform for each reference pattern.
        ReferenceDataList[i] = createReferencePatternWithTuningCorrection(ReferenceDataList[i], verbose=True)
